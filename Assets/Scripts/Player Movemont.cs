...
public class PlayerMovemont : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //G키로 글라이더 제어(누르는 동안 활성화됨)
        if(Input.GetKey(KeyCode.G) && !isGrounded && gliderTimeLeft >0)        //G키를 누르면서 땅에 있지 않고 글라이더의 남은 시간이 있을 때(3가지 조건)
        {
        ...
        //점프 입력
        if (Input.GetButtonDown("Jump") && isGrounded)            //&& 두 값을 만족할때 -> (스페이스 버튼을 눌렀을 때와 isGrouded가 True일때) 
        {
    ...
    private void OnCollisionExit(Collision collision)             //충돌 처리 함수
    {
        if (collision.gameObject.tag == "Ground")                  //충돌이 일어난 물체의 Tag가 Ground인 경우
        {
            realGrouned = true;                                     //땅과 충돌하면 True로 변경한다.
