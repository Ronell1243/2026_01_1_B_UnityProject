...
public class MyCharacer : MonoBehaviour
{
    ...
    void Update()
    {
        Timer = Timer - Time.deltaTime;      //시간을 매 프레임마다 감소시킨다.(deltaTime은 프레임간의 시간 간격을 의미함.)

        if (Timer <= 0)                     //만약 타이머의 수치가 0이하로 내려갈 경우
        {
            Timer = 1.0f;                   //다시 1초로 변경시켜준다.
            Health = Health - 20;           //1마다 체력이 20 줄어든다.
        }
        ...
        {
            Destroy(this.gameObject);       //이 오브젝트를 없앤다.
        }
    }
}
